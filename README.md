...
- 可以使用`geopy`库或者基于坐标转换实现坐标漂移，算法采用坐标转换的方式
- 算法内部设置了噪声等级参数`noise_level`默认为`low`（此时的定位精度为20）
- 噪声等级越高，定位精度越低（定位精度对应的数值越大，即轨迹点在更大的范围内偏移）



| 噪声等级 | 定位精度 | 备注                                             |
| -------- | -------- | ------------------------------------------------ |
| 低       | 高，20m  | 相当于采集时仅保留GPS定位点                      |
| 中       | 中，50m  | 相当于采集时保留GPS定位点和精度较高的Wi-Fi定位点 |
| 高       | 低，100m | 相当于采集时保留GPS定位点和大部分Wi-Fi定位点     |



![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/a563b5c46b074b01adeb2e621beceb7c.png)
